...
void ClipMesh::Update ()
{
    ...
    int iNegQuantity = (int)kNegIndices.size();
    for (i = 0; i < iNegQuantity; i++)
    {
        m_spkTorus->VBuffer->Color3(0,kNegIndices[i]).B() = 1.0f;
    }
    int iPosQuantity = (int)kPosIndices.size();
    for (i = 0; i < iPosQuantity; i++)
    {
        m_spkTorus->VBuffer->Color3(0,kPosIndices[i]).R() = 1.0f;
    }

    // To display the triangles generated by the split.
    m_spkTorus->IBuffer = WM4_NEW IndexBuffer(iNegQuantity + iPosQuantity);
    memcpy(m_spkTorus->IBuffer->GetData(),&kNegIndices[0],
        iNegQuantity*sizeof(int));
    memcpy(m_spkTorus->IBuffer->GetData() + iNegQuantity, &kPosIndices[0],
        iPosQuantity*sizeof(int));
}
//----------------------------------------------------------------------------

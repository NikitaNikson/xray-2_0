    ...
    Texture* pkTexture)
{
    ...
    if (aucSrc)
    {
        ...
        {
            // Pad to an RGBA image.  The A channel is set to zero.
            iQuantity = pkImage->GetQuantity();
            iByteQuantity = 16*iQuantity;
            aucRSrc = WM4_NEW unsigned char[iByteQuantity];
            float* afSrc = (float*)aucSrc;
            float* afRSrc = (float*)aucRSrc;

    ...
    for (i0 = 0; i0 < 3; i0++)
    {
        for (i1 = 0; i1 < 3; i1++)
        {
            ...
            if (kAxis.SquaredLength() <= Math<Real>::ZERO_TOLERANCE)
            {
                ...
                FindContactSet<Real>(*m_pkBox0,*m_pkBox1,eSide,kBox0Cfg,
                    kBox1Cfg,rkVelocity0,rkVelocity1,m_fContactTime,
    ...
    const Vector3<Real>& rkRotCenter1, const Vector3<Real>& rkRotAxis1)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        kSubBox0.Extent[i] = m_pkBox0->Extent[i];
        kSubBox1.Axis[i] = m_pkBox1->Axis[i];
        kSubBox1.Extent[i] = m_pkBox1->Extent[i];
    }

    ...
    m_kPlaneOrigin(Vector3<Real>::ZERO)
{
    ...
    Tetrahedron* pkTetra = WM4_NEW Tetrahedron(m_aiSV[0],
    ...
    std::map<Tetrahedron*,int> kPermute;
    typename std::set<Tetrahedron*>::iterator pkTIter =
...
bool Delaunay3<Real>::GetHull (int& riTQuantity, int*& raiIndex) const
{
    ...
    // V[3] = (0,0,1) with no adjacent tetrahedra.  The m_aiIndex[] array
    // is <0,1,2,3>.
    //   i = 0, face = 0:
...
void Delaunay3<Real>::Update (int i)
{
    ...
    Tetrahedron* pkTetra = GetContainingTetrahedron(i);

    // Locate and remove the tetrahedra forming the insertion polyhedron.
    std::stack<Tetrahedron*> kStack;
    ETManifoldMesh kPolyhedron(0,Face::TCreator);
    kStack.push(pkTetra);
    ...
    {
        pkFace = (Face*)pkTIter->second;

        // Create and insert the new tetrahedron.
        pkTetra = WM4_NEW Tetrahedron(i,pkFace->V[0],pkFace->V[1],
            pkFace->V[2]);
        m_kTetrahedron.insert(pkTetra);
...
Delaunay3<Real>::GetContainingTetrahedron (int i) const
{
    ...
    Tetrahedron* pkTetra = *m_kTetrahedron.begin();
...
void Delaunay3<Real>::RemoveTetrahedra ()
{
    ...
    std::set<Tetrahedron*> kRemoveTetra;
    Tetrahedron* pkTetra;
    typename std::set<Tetrahedron*>::iterator pkTIter =
        m_kTetrahedron.begin();
    for (/**/; pkTIter != m_kTetrahedron.end(); pkTIter++)
...
//----------------------------------------------------------------------------
template <class Real>
bool Delaunay3<Real>::SharesFace (int i, Tetrahedron* pkFace,
    Tetrahedron* pkAdj)
{
...
//----------------------------------------------------------------------------
template <class Real>
Delaunay3<Real>::Tetrahedron::Tetrahedron (int iV0, int iV1, int iV2, int iV3)
{
    V[0] = iV0;
    V[1] = iV1;
    V[2] = iV2;
...
bool Delaunay3<Real>::Tetrahedron::IsInsertionComponent (int i,
    Tetrahedron* pkAdj, const Query3<Real>* pkQuery, const int* aiSupervertex)
{
...
//----------------------------------------------------------------------------
template <class Real>
int Delaunay3<Real>::Tetrahedron::DetachFrom (int iAdj, Tetrahedron* pkAdj)
{
...
//----------------------------------------------------------------------------
template <class Real>
Delaunay3<Real>::Face::Face (int iV0, int iV1, int iV2, int iNullIndex,
    Tetrahedron* pkTetra)
    :
...
//----------------------------------------------------------------------------
template <class Real>
ETManifoldMesh::TPtr Delaunay3<Real>::Face::TCreator (int iV0, int iV1,
    int iV2)
{
    return WM4_NEW Face(iV0,iV1,iV2,0,0);
}
//----------------------------------------------------------------------------

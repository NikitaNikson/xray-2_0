    ...
    Query::Type eQueryType, Real fEpsilon, Indices& rkTriangles)
{
    ...
    const int* aiIndex = 0;
    InitializeVertices(iVQuantity,aiIndex);
    ...
    std::vector<int>& rkTriangle)
{
    ...
    while (bRemoveAnEar)
    {
        ...
        else
        {
            ...
            {
                // In theory, if the current vertex is convex (the ear that is
                // being removed) and the previous vertex is convex, after the
                // ear removal the previous vertex must remain convex.  That
                // is, the call to IsConvex(iVPrev) must return true.  In
                // practive when using QT_REAL, floating-point round-off
                // errors can cause the determinant calculated in ToLine (in
                // the IsConvex(iVPrev) call) to be small and negative, in
                // which case IsConvex(iVPrev) returns false.  If you trigger
                // this assert, you have run into this problem.  Try using
                // QT_FILTERED with a small epsilon.
                //
                // TODO:  Add the recalculation using exact arithmetic here to
        ...
        else
        {
            ...
            {
                // In theory, if the current vertex is convex (the ear that is
                // being removed) and the next vertex is convex, after the
                // ear removal the next vertex must remain convex.  That
                // is, the call to IsConvex(iVNext) must return true.  In
                // practive when using QT_REAL, floating-point round-off
                // errors can cause the determinant calculated in ToLine (in
                // the IsConvex(iVNext) call) to be small and negative, in
                // which case IsConvex(iVNext) returns false.  If you trigger
                // this assert, you have run into this problem.  Try using
                // QT_FILTERED with a small epsilon.
                //
                // TODO:  Add the recalculation using exact arithmetic here to
    ...
    const Indices& rkInner, IndexMap& rkMap, Indices& rkCombined)
{
    ...
    int i0, i1;
    Real fS = Math<Real>::MAX_REAL, fT = Math<Real>::MAX_REAL;
    ...
    // The intersection kIntr[0] stored only the t-value of the ray.  The
    // actual point is (mx,my)+t*(1,0), so kIntr[0] must be adjusted.
    kIntr[0] += kM[0];
...
//----------------------------------------------------------------------------
template <class Real>
void TriangulateEC<Real>::RemoveE (int i)
{
...
//----------------------------------------------------------------------------
template <class Real>
void TriangulateEC<Real>::RemoveEFirst ()
{
    assert(V(m_iEFirst).IsEar);
    V(m_iEFirst).IsEar = false;
...
void TriangulateEC<Real>::RemoveR (int i)
{
    ...
    else
    {
        ...
        {
            // TODO.  In theory, we should not get here.  In practice,
            // floating-point round-off errors can cause problems.  See the
            // comments in DoEarClipping() about the IsConvex() function.
            // If you trigger this assert, use QT_FILTERED with a small
            // epsilon instead of QT_REAL.  I need to fix this internally
            // by adding more code in DoEarClipping() that will prevent
            // this assert from triggering.
            assert(false);
        }
    }
...
//----------------------------------------------------------------------------
template <class Real>
void TriangulateEC<Real>::ValidateReflexList () const
{
    int i;
    ...
    {
        assert(m_iRLast >= 0);
        if (m_iRFirst == m_iRLast)
        {
            assert(!m_kVertex[m_iRFirst].IsConvex);
            for (i = 0; i < (int)m_kVertex.size(); i++)
            {
                if (i != m_iRFirst)
                {
                    assert(m_kVertex[i].IsConvex);
                }
            }
        }
        else
        {
            std::set<int> kRIndices;
            i = m_iRFirst;
            do
            {
                assert(!m_kVertex[i].IsConvex);
            ...
            while (i != m_iRFirst && i != -1);

            for (i = 0; i < (int)m_kVertex.size(); i++)
            {
                std::set<int>::const_iterator pkIter = kRIndices.find(i);
                if (pkIter == kRIndices.end())
                {
                    assert(m_kVertex[i].IsConvex);
                }
            }
...
//----------------------------------------------------------------------------
template <class Real>
void TriangulateEC<Real>::ValidateEarList () const
{
    int i;
    ...
    {
        assert(m_iELast >= 0);
        if (m_iEFirst == m_iELast)
        {
            assert(m_kVertex[m_iEFirst].IsEar);
            for (i = 0; i < (int)m_kVertex.size(); i++)
            {
                if (i != m_iEFirst)
                {
                    assert(!m_kVertex[i].IsEar);
                }
            }
        }
        else
        {
            std::set<int> kEIndices;
            i = m_iEFirst;
            do
            {
                assert(m_kVertex[i].IsEar);
            ...
            while (i != m_iEFirst && i != -1);

            for (i = 0; i < (int)m_kVertex.size(); i++)
            {
                std::set<int>::const_iterator pkIter = kEIndices.find(i);
                if (pkIter == kEIndices.end())
                {
                    assert(!m_kVertex[i].IsEar);
                }
            }

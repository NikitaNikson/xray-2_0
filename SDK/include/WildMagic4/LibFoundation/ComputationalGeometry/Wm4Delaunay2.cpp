    ...
    m_kLineDirection(Vector2<Real>::ZERO)
{
    ...
    Triangle* pkTri = WM4_NEW Triangle(m_aiSV[0],m_aiSV[1],m_aiSV[2]);
...
void Delaunay2<Real>::Update (int i)
{
    ...
    Triangle* pkTri = GetContainingTriangle(i);
    ...
    for (pkEIter = rkEMap.begin(); pkEIter != rkEMap.end(); pkEIter++)
    {
        ...
        pkTri = WM4_NEW Triangle(i,pkEdge->V[0],pkEdge->V[1]);
        m_kTriangle.insert(pkTri);
...
//----------------------------------------------------------------------------
template <class Real>
Delaunay2<Real>::Triangle::Triangle (int iV0, int iV1, int iV2)
{
    V[0] = iV0;
    V[1] = iV1;
    V[2] = iV2;
...
//----------------------------------------------------------------------------
template <class Real>
bool Delaunay2<Real>::Triangle::IsInsertionComponent (int i, Triangle* pkAdj,
...
//----------------------------------------------------------------------------
template <class Real>
int Delaunay2<Real>::Triangle::DetachFrom (int iAdj, Triangle* pkAdj)
{
...
//----------------------------------------------------------------------------
template <class Real>
Delaunay2<Real>::Edge::Edge (int iV0, int iV1, int iNullIndex,
    Triangle* pkTri)
    :
...
//----------------------------------------------------------------------------
template <class Real>
VEManifoldMesh::EPtr Delaunay2<Real>::Edge::ECreator (int iV0, int iV1)
{

    ...
    const Vector3<Real>* akVertex, int iTQuantity, const int* aiIndex)
{
    ...
    for (i = 0; i < m_iVQuantity; i++)
    {
        ...
        for (iRow = 0; iRow < 3; iRow++)
        {
            for (iCol = 0; iCol < 3; iCol++)
            {
                Real fAbsEntry = Math<Real>::FAbs(akDWTrn[i][iRow][iCol]);
                if (fAbsEntry > fMaxAbs)
                {
                    fMaxAbs = fAbsEntry;
                }
            }
        }
        if (fMaxAbs < (Real)1e-07)
        {
            abDWTrnZero[i] = true;
        }
    ...
    for (i = 0; i < m_iVQuantity; i++)
    {
        ...
        if (abDWTrnZero[i])
        {
            ...
            m_afMinCurvature[i] = (Real)0;
            m_afMaxCurvature[i] = (Real)0;
            m_akMinDirection[i] = kU;
            m_akMaxDirection[i] = kV;
            continue;
        }

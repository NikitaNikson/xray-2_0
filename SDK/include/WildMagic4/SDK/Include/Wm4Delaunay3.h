...
private:
    ...
    class WM4_FOUNDATION_ITEM Tetrahedron
    {
    public:
        Tetrahedron (int iV0, int iV1, int iV2, int iV3);

        bool IsInsertionComponent (int i, Tetrahedron* pkAdj,
            const Query3<Real>* pkQuery, const int* aiSupervertex);
        int DetachFrom (int iAdj, Tetrahedron* pkAdj);
    ...
    class WM4_FOUNDATION_ITEM Face : public ETManifoldMesh::Triangle
    {
    public:
        Face (int iV0 = -1, int iV1 = -1, int iV2 = -1, int iNullIndex = -1,
            Tetrahedron* pkTri = 0);
    ...
    bool SharesFace (int i, Tetrahedron* pkFace, Tetrahedron* pkAdj);
    ...
    std::set<Tetrahedron*> m_kTetrahedron;

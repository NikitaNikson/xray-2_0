...
//	return	dot(t_lmh,1.h/4.h);
	float4	t_lmh = t_hemi.Sample( smp_rtlinear, tc);
	return	t_lmh.a;
}
...
#ifndef USE_MSAA
	float4 P	= t_position.Sample( smp_nofilter, tc );
#else
...
#ifndef USE_MSAA
	float4 N	= t_normal.Sample( smp_nofilter, tc );
#else
	float4 N	= t_normal.Load( int3( tc * pos_decompression_params2.xy, 0 ), iSample );
#endif
...
#ifndef USE_MSAA
	float4	C	= t_diffuse.Sample(  smp_nofilter, tc );
#else
	float4	C	= t_diffuse.Load( int3( tc * pos_decompression_params2.xy, 0 ), iSample );
#endif

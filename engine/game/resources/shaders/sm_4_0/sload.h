...
{
   return	t_base.Sample( smp_base, tc);
}
	...
	if (I.position.z < fParallaxStopFade)
	{
		...
		for( int i=0; i<nNumSteps; ++i )
		{
			...
			{	
				...
				fCurrHeight = t_bumpX.SampleLevel( smp_base, vTexCurrentOffset.xy, 0 ).a; 
...
*/
		...
		vTexCurrentOffset -= vTexOffsetPerStep;
		float fPrevHeight = t_bumpX.Sample( smp_base, float3(vTexCurrentOffset.xy,0) ).a;
...
void UpdateTC( inout p_bumped I)
{
	...
	float	height	= t_bumpX.Sample( smp_base, I.tcdh).w;	//
			//height  /= 2;
			//height  *= 0.8;
...
surface_bumped sload_i( p_bumped I)
{
	...
	float4 	Nu	= t_bump.Sample( smp_base, I.tcdh );		// IN:	normal.gloss
	float4 	NuE	= t_bumpX.Sample( smp_base, I.tcdh);	// IN:	normal_error.height

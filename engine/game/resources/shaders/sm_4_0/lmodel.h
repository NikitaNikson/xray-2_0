...
//	return tex3D 		(s_material,	float3( dot(L,N), dot(H,N), m ) );		// sample material
	return t_material.Sample( smp_material, float3( dot(L,N), dot(H,N), m ) ).xxxy;		// sample material
}
/*
float plight_infinity2( float m, float3 pnt, float3 normal, float3 light_direction )
{
	...
	float 	f 	= saturate(dot(-V,R));
			s  *= f;
	float4	r	= tex3D 			(t_material,	float3( dot(L,N), s, m ) );	// sample material
...
//	float4 light	= tex3D		(s_material, float3( dot(L,N), dot(H,N), m ) ); 	// sample material
	float4 light	= t_material.Sample( smp_material, float3( dot(L,N), dot(H,N), m ) ).xxxy;		// sample material
  	return att*light;
}

...
namespace
{
	...
	static D3DXVECTOR3 * Vec3Normalize( D3DXVECTOR3 *pOut, CONST D3DXVECTOR3 *pV ) {
		...
		float v = 1.0f / xray::math::sqrt(pV->x * pV->x + pV->y * pV->y + pV->z * pV->z);
		pOut->x *= v;
		pOut->y *= v;
		pOut->z *= v;
		return pOut;
								...
								D3DXVECTOR3 workingPosition)
{
		...
		vector<D3DXVECTOR3> groupNormalVectors;
		...
		for( i = 0; i < neighborGroups.size(); ++i )
		{
			...
			R_ASSERT(curGroup.size()!=0 && "should not be a zero group here.");
											...
											vector< Vertex >& theVerts)
{
	...
	R_ASSERT(triA && triB && "invalid data passed to SharesEdgeNoSplit");
...
void MeshMender::OrthogonalizeTangentsAndBinormals( vector< Vertex >&   theVerts )
{
...
void MeshMender::UpdateTheIndicesWithFinalIndices( vector< u16 >& theIndices )
{
	...
	{
		theIndices[oIndex+0] = (u16)m_Triangles[i].indices[0];
		theIndices[oIndex+1] = (u16)m_Triangles[i].indices[1];
		theIndices[oIndex+2] = (u16)m_Triangles[i].indices[2];
		oIndex				+= 3;
	}
}
										...
										vector< u16 >& mappingNewToOldVert)
{
	...
	{
		//for each triangle
		set<u32> alreadyDuped;

		
		for( u32 begin = 0 ; begin < 3 ; ++begin)
		{
			...
			if( sBegin <= 1.0f  && sEnd <= 1.0f  && sBegin >= 0.0f  && sEnd >= 0.0f  )
			{
				...
				if( xray::math::abs(sBegin - sEnd) > 0.5f )
				{
					u32 theOneToDupe = begin;
					...
					if(alreadyDuped.find(theOneToDupe) == alreadyDuped.end())
					{
						...
						theDupe.s += 1.0f;
						theIndices[index + theOneToDupe] = (u16)theVerts.size();
			...
			{
				//we only handle coordinates between 0 and 1 for the cylindrical wrappign fix
				if( xray::math::abs(tBegin - tEnd) > 0.5f )
				{
					u32 theOneToDupe = begin;
					//we have some wrapping going on.
					if(tBegin > tEnd)
									...
									vector< u16 >& mappingNewToOldVert)
{
	if( oldIndex >= originalNumVerts )
	{
		...
		u16 originalVertIndex = mappingNewToOldVert[oldIndex];

...
inline D3DXVECTOR3& cv_vector ( D3DXVECTOR3	&l, const float3& r  )
{
	l.x = r.x;
	l.y = r.y;
	l.z = r.z;
...
inline float3&  cv_vector (  float3& l, const D3DXVECTOR3	&r  )
{
	l.x = r.x;
	l.y = r.y;
	l.z = r.z;
...
class MeshMender
{
	...
	public:
		...
		bool Mend( 		  vector< Vertex >&    theVerts,
	...
	protected:
		...
		void SetUpData(	vector< Vertex >&		theVerts,
		...
		typedef vector<TriID> TriangleList;
		...
		typedef map<D3DXVECTOR3, TriangleList > VertexChildrenMap;
		...
		//eachother
		typedef vector<TriangleList > NeighborGroupList;
		...
		void OrthogonalizeTangentsAndBinormals( vector< Vertex >&   theVerts );

		void UpdateTheIndicesWithFinalIndices( vector< u16 >& theIndices );

		void FixCylindricalWrapping(	vector< Vertex >& theVerts , 
										vector< u16 >& theIndices,
										vector< u16 >& mappingNewToOldVert);

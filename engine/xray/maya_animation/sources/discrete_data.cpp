...
public:
	...
	virtual void				add_frame			(u32 bone, frame const& frame );
...
discrete_data_impl::discrete_data_impl	(): m_time_scale( default_fps ), m_object_mover_id( u32(-1) )
{
	m_precision[ channel_translation_x ] = 0.005f;
	m_precision[ channel_translation_y ] = 0.005f;
	m_precision[ channel_translation_z ] = 0.005f;

	m_precision[ channel_rotation_x ] = 0.005f;
	m_precision[ channel_rotation_y ] = 0.005f;
	m_precision[ channel_rotation_z ] = 0.005f;

	m_precision[ channel_scale_x ] = 0.005f;
	m_precision[ channel_scale_y ] = 0.005f;
	m_precision[ channel_scale_z ] = 0.005f;
}
...
//	std::fill( m_angle_normalize_base.begin(), m_angle_normalize_base.end(), 0.f );
	...
	for( u32 i = 0; i < cnt; ++i )
	{
		...
		if( i > 0 )
		{
			...
			to_close_xyz( f_object_mover_bone.rotation, prev_object_mover_bone.rotation, math::pi_d2 );
			object_mover_bone_data.set_frame( i, f_object_mover_bone );

			frame f_prev_object_mover = object_mover_data.get_frame( i - 1 );
			to_close_xyz( f_object_mover.rotation, f_prev_object_mover.rotation, math::pi_d2 );
...
bool	do_fix_angle_normalize( float a0,  float &a1, float &saved_angle_base )
{
	...
	if( !math::similar( dist, angular_dist, math::epsilon_3 ) ) 
	{
...
void	discrete_data_impl::add_frame(u32 bone, frame const& frm ) 
{
...
u32	discrete_data_impl::frame_count( )const 
{
	...
	for(u32 i = 0; i < bone_cnt; ++i )
		...
		{
			ASSERT( data.bone_animation( i ).m_channels[j].size() == frame_cnt );
		}
	return frame_cnt;
}
...
void build_channel( i_animation_channel_data::i_data_channel_type &channel, float epsilon, const float *p, u32 size,  xray::maya_animation::raw_bi_spline_1d *raw_bi_spline )
{
...
void build_channel( u32 bone, u32 ch, float epsilon,
					...
					i_skeleton_animation_data &animation_data,
					xray::maya_animation::raw_bi_spline_1d *raw_bi_spline
					)
{
	xray::animation::i_animation_channel_data::i_data_channel_type &channel =
			animation_data.bone( bone).channel( (enum_channel_id)(ch) ).bi_spline();
...
void	maya_animation_world::build_animation_data( const discrete_data &data, i_skeleton_animation_data &animation_data   )const
{
	u32 denormals, control_word;
	set_fp_state						( denormals, control_word );

	discrete_data_impl::discrete_bone_data const& impl_data 
			= dynamic_cast<const discrete_data_impl&> ( data ).get_data();

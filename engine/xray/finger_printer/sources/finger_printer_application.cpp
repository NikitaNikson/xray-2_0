...
// bit_offs1, bit_offs2 <= 4
void	read_finger_print_first_method	(pstr mark, buffer_string * out_finger_print, int bit_offs1, int bit_offs2, unsigned int build_id)
{
...
void	read_finger_print_second_method (pstr mark, buffer_string * out_finger_print)
{
	bool differs_from_original		=	false;
...
{
	FILE * file							=	NULL;
	if ( !fs::open_file(& file, open_file_read | open_file_write, file_name) )
	{
	...
	* out_file_data						=	ALLOC(char, file_size + 1024);
	...
	if ( fread_s(* out_file_data, file_size, 1, file_size, file) != file_size )
	{
...
void   application::initialize ()
{
	...
	xray::core::initialize					("main", core::create_log, core::perform_debug_initialization);
}
...
void   application::do_finger_printing (fixed_string512 const & file_name, fixed_string512 const & finger_print_message, pstr mark1, pstr mark2, u32 build_id, pstr file_data, u32 file_size)
{
	...
	if ( s_read_key )
	{
		...
		if ( mark1 )
  			read_finger_print_first_method	(mark1, & finger_print1, 3, 2, build_id);

		fixed_string512	finger_print2	=	"mark is not available";
		if ( mark2 )
  			read_finger_print_second_method	(mark2, & finger_print2);
...
void   application::execute ()
{
	...
	else
	{
		...
		ask_build_id_line				+=	" -print_build_id";

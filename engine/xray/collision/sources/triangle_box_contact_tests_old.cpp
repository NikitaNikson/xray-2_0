...
{
	float sqr_magnitude = (v|v);
	if( sqr_magnitude < math::epsilon_7 )
		return false;
	float	l	=	1.f/sqrt(sqr_magnitude); //dRecipSqrt(sqr_magnitude);
		v[0]		*=	l;
		v[1]		*=	l;
		v[2]		*=	l;
	return true;
}
void	triangle_box_contact_tests::test( buffer_vector<u32> &backs, const u32 * ind, float3 const* verts )
{
/////////////////////////////////////////////////////////////////////////////////////	
	...
	triangle_mesh_primitive_contact_tests::test( backs, ind, verts );
...
/////////////////NOT REFACTORED//////////////////
/////////////////NOT REFACTORED//////////////////
  const float3 &hside = m_geometry.half_sides();
 ...
 // if (maxc > 3) maxc = 3;	// no more than 3 contacts per box allowed
  ...
  float tri_ax_sq_mag = res.tri_ax.square_magnitude();
